...
/** For a slot form, write the matching row into the slot's production table so the
 *  submission appears in the production lists/cards like the default form. Line and PO
 *  are auto-detected; production_mapping supplies the numeric columns. The original
 *  custom submission id is stored in custom_data so the row's detail can be driven by
 *  the form. Non-fatal: never blocks the custom submission. */
async function writeProductionRow(
  config: CustomFormConfig, values: Record<string, unknown>, userId: string, submissionId: string,
  ...
  if (!lineName || !poNumber) {
    return { written: false, reason: "This form needs a Line field and a PO field for its data to appear in the production lists." };
  }
 ...
 *  matching production row so it appears in the production lists/cards. */
export async function submitCustomForm(
  ...
  const { data: inserted, error } = await supabase.from("custom_form_submissions" as never).insert({
  ...
  const production = await writeProductionRow(config, values, userId, (inserted as { id: string }).id);
