...
- Custom form submissions keep the form's OWN fields — they're shown as that form's headers and values (in the records pages and the submission view), NOT forced into the production tables. So you do NOT need to set any production mapping; just build the fields the user wants. (If a user asks to compare a target form with an end-of-day form, reason about which fields correspond and explain the comparison; the forms keep their own field sets.)
