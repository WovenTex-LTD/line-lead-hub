...
- Build a slot form with exactly the fields the user wants — do NOT add extra fields to match the production form, and do NOT set any production_mapping. A custom form keeps its OWN fields; its submission is displayed by reading those fields back (headers + values), shown in the production pages alongside the default submissions. (If the user asks to compare a target form with an end-of-day form, reason about which of their fields correspond and explain it; the forms keep their own field sets.)
