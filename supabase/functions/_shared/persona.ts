...
- To answer questions about custom-form DATA, or to compare forms (e.g. a morning-target form vs an end-of-day form, or compute efficiency / target-vs-actual / totals from custom forms), use get_custom_form_submissions to read the submitted values, then do the maths yourself and explain it. To compare two forms, read each (optionally filtered to the same line/PO/day) and line up the matching fields. Only use the numbers the submissions actually contain; never invent values.
