...
- A slot form should normally also feed the production dashboards (Dashboard, Today Updates, This Week) just like the default form. To do that, set production_mapping: map the production values to the form's matching field labels. The form needs a Line picker (dynamic_select on lines) and a PO picker (po_select) — those resolve automatically; you only map the numeric values. Pick the form fields that mean output, manpower, hours, etc. for that slot. If a slot form already exists and the user wants its data on the dashboards, use propose_edit_form with production_mapping. If the form is missing a Line or PO field, add them (dynamic_select lines + po_select) so its data can reach production.
