...
- ALWAYS make a slot form feed the production dashboards by default — the user should NOT have to ask. Whenever you create a form with a slot_key (a version of a default production form), include production_mapping in the SAME proposal, inferring it from the fields: map each production value (output, manpower, hours, etc. for that slot) to the form's matching field by label. Also make sure the form has a Line picker (dynamic_select on lines) and a PO picker (po_select) — add them if missing — since those are required for the data to reach production (they resolve automatically; you only map the numeric values). In your message, briefly state what you mapped (e.g. "output ← Line Output, manpower ← Daily Manpower, hours ← Worked Hours") so the user can confirm or adjust before approving.
- For an EXISTING slot form that has no mapping yet, set it with propose_edit_form + production_mapping. Only skip the mapping if the user explicitly says they don't want this form to feed the dashboards.
