-- Bulletproof production write for custom slot forms.
-- A custom form's submission is written into its slot's production table by this
-- function, which INTROSPECTS the table and fills any NOT-NULL/no-default column the
-- form didn't supply with a type-appropriate default — so a future schema change
-- (a new required column) can never break the write, with zero code changes.
-- SECURITY INVOKER: runs as the calling user, so the existing RLS on the production
-- tables is the authority. Locked to an allowlist of production tables.
-- Safe to re-run.
...
CREATE OR REPLACE FUNCTION public.upsert_custom_production_row(
  ...
  p_values          jsonb,   -- the form's mapped numeric columns { column: value }
...
AS $$
DECLARE
  allowed   text[] := ARRAY['sewing_targets','sewing_actuals','cutting_targets','cutting_actuals','finishing_targets','finishing_actuals'];
  full_vals jsonb;
  upd_vals  jsonb;
  rec       record;
  existing_id uuid;
...
BEGIN
  ...
  -- One row per natural key: update the supplied columns, else insert.
