-- Finance Portal: Invoicing module
-- invoices: one invoice per buyer/shipment, linked optionally to a work order
-- invoice_line_items: line items (style/qty/price) within an invoice
...
-- Indexes
CREATE INDEX invoices_factory_id_idx ON invoices(factory_id);
CREATE INDEX invoices_status_idx ON invoices(factory_id, status);
...
COMMENT ON TABLE invoices IS 'Finance Portal: commercial invoices issued to buyers';
COMMENT ON TABLE invoice_line_items IS 'Finance Portal: line items within an invoice';
